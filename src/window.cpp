...
void window::create_ui()
{
	...
	_quality_choice->value(1);
	_quality_choice->hide();

	Fl_Button *author = new Fl_Button(10, 55, 380, 15,
		"Press ... and select your pdf file.\nThen press GO and wait (may take some time).");
	author->box(FL_FLAT_BOX);
	author->align(FL_ALIGN_INSIDE);
	author->labelsize(12);
	author->labelcolor(fl_rgb_color(0, 0, 0));
	author->deactivate();
	//author->hide();
}
